...
{
    public class ButterflySpawn_KG : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            _area = GetComponentInParent<ButterflySpawnArea_KG>();
            ...
            // set the flying speed
             _animator.speed = flyingSpeed; // Adjust animation speed 
            ...
            if (!_isPlaying)
            {
                if (_cooldown <= 0)
                {
                    ...
                    transform.position = Vector3.Lerp(
                        transform.position,
                        new Vector3(
                            Random.Range(_area.Collider.bounds.min.x, _area.Collider.bounds.max.x),
                            Random.Range(_area.Collider.bounds.min.y, _area.Collider.bounds.max.y),
                            Random.Range(_area.Collider.bounds.min.z, _area.Collider.bounds.max.z)
                        ),
                        Time.deltaTime * flyingSpeed // Apply flying speed here
                    );
